...
class SparepartSaleResource extends Resource
{
    ...
    {
        $harga_subtotal = floatval($get('harga_unit')) * floatval($get('jumlah_unit'));

        $is_pajak = Sparepart::find($get('sparepart_id'))->is_pajak;
        if ($is_pajak == 1) {
    ...
    {
            $selectedSparepart = collect($get('SparepartDSale'))->filter(fn ($item) => !empty($item['sparepart_id']));
                            ...
                            ->schema([
                                ...
                                ->afterStateUpdated(
                                    function (Set $set, $state) {
                                        ...
                                        $sparepart = SparepartSatuans::where();
                                    }
                                ),
                                ...
                                //             $harga_unit = SparepartSatuans::where(['sparepart_id' => $get('sparepart_id'), 'satuan_id' => $get('satuan_id')])->get()->first();
                                //             $set('harga_unit', $harga_unit->harga);
                                ...
                                // ->numeric()
                                // ->readOnly()
                                // ->disabled(fn (Get $get) => !$get('satuan_id')),
